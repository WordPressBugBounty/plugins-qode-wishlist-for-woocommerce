...
class Qode_Wishlist_For_WooCommerce_Admin_Options_Custom_Page {
	...
	public function render() {
		...
		echo qode_wishlist_for_woocommerce_framework_get_template_part( QODE_WISHLIST_FOR_WOOCOMMERCE_ADMIN_PATH . '/inc', 'admin-pages/options-custom-pages/' . $this->slug, 'templates/' . $this->slug, '' );
	}
	...
	public function enqueue_scripts() {
		// phpcs:ignore WordPress.Security.NonceVerification
		if ( true === $this->get_style() && isset( $_GET['page'] ) && QODE_WISHLIST_FOR_WOOCOMMERCE_MENU_NAME === sanitize_text_field( wp_unslash( $_GET['page'] ) ) && isset( $_GET['template'] ) && $this->get_slug() === sanitize_text_field( wp_unslash( $_GET['template'] ) ) ) {
			// phpcs:ignore WordPress.WP.EnqueuedResourceParameters
